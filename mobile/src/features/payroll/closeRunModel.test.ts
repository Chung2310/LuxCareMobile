...
it("requires review status, version and management scope", () => {
  ...
  for (const version of [-1, NaN, 0.5])
    expect(canClosePayrollRun(user, "b", { ...run, version })).toBe(false);
...
it("posts expectedVersion to encoded close endpoint", async () => {
  ...
  await createPayrollService({ fetch, getAccessToken: () => "t" }).closeRun(
    "r/1",
    4,
  );
...
it.each([
  ...
  "PAYROLL_REVISION_MISSING",
  "PAYROLL_VERSION_CONFLICT",
])("preserves %s without retrying close", async (code) => {
  const fetch = vi
    .fn()
    .mockResolvedValue(new Response(JSON.stringify({ code }), { status: 409 }));
  await expect(
    createPayrollService({ fetch, getAccessToken: () => "t" }).closeRun("r", 4),
