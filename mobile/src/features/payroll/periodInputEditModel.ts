...
export function periodInputEditPayload(item: PeriodInput, values: Record<string, string>, reason: string) {
  ...
  if (!Object.keys(changes).length) throw new Error("Chưa có giá trị thay đổi. Ô trống giữ giá trị cũ.");
  return { ...changes, expectedVersion: item.version, reason: reason.trim() };
}
export function validateEditedPeriodInput(
  ...
  if (
    ...
    (Object.keys(periodInputFields) as Field[]).some((key) => result[key] !== (payload[key] ?? original[key])) ||
