...
export function ReopenPayrollRun({
  ...
  const allowed = canReopenPayrollRun(
    user,
    ...
    try {
      ...
      if (active.current) {
        showAlert(
          ...
          "Kỳ lương đã được chuyển về trạng thái nháp và cập nhật số liệu.",
          [{ text: "Đã hiểu" }],
          "success",
        );
      }
    ...
    <Card>
      ...
      {done ? (
        <Text style={styles.text}>
          Đã mở lại kỳ về nháp. Kiểm tra số liệu trước khi chỉnh sửa hoặc tính
          lại.
        </Text>
      ) : (
        <>
          <Text style={styles.text}>
            Mở lại sẽ đưa kỳ về nháp và bỏ bản số liệu đã lưu khi duyệt. Phiếu
            lương đã phát hành sẽ không còn trong danh sách của nhân viên khi kỳ
            ở trạng thái nháp.
          </Text>
          <Text style={styles.muted}>
            Kỳ có thanh toán đã xác nhận cần xử lý thanh toán trước; kỳ đã trả
            không thể mở lại. Các bản phiếu đã tải/chia sẻ vẫn có thể tồn tại.
          </Text>
          <Field
          ...
          {!confirming ? (
            ...
            <>
              <Text style={styles.text}>
                Xác nhận mở lại kỳ với lý do: {reason.trim()}
              </Text>
              <Button
