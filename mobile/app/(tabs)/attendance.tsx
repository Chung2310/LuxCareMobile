    ...
    try {
      ...
      if (!perm.granted) {
        ...
        if (!requestRes.granted) {
          ...
          showAlert("Cần quyền vị trí", "LuxCare cần quyền truy cập vị trí thiết bị để xác thực bạn đang có mặt tại cơ sở/chi nhánh khi chấm công.", [{ text: "Mở Cài đặt", onPress: () => { void Linking.openSettings().catch(() => showAlert("Không thể mở Cài đặt", "Vui lòng mở Cài đặt trên điện thoại để cấp quyền vị trí.", undefined, "error")); } }, { text: "Đã hiểu", style: "cancel" }], "info");
          return;
        }
      }
      ...
      if (!servicesEnabled) {
        setSubmittingAction(null);
        setSubmittingStep(null);
        showAlert("Dịch vụ định vị đang tắt", "Vui lòng bật dịch vụ định vị (GPS) trên điện thoại của bạn để thực hiện chấm công.", undefined, "info");
        return;
      }
      ...
      showAlert(type === "check-in" ? "Chấm công vào thành công!" : "Chấm công ra thành công!", type === "check-in"
            ? `Đã ghi nhận Chấm công vào ca lúc ${timeNow}. Chúc bạn một ca làm việc tràn đầy năng lượng!`
            : `Đã ghi nhận Chấm công ra ca lúc ${timeNow}. Hẹn gặp lại bạn vào ca làm việc tiếp theo!`, undefined, "success");
      ...
      showAlert("Không thể chấm công", msg ||
          "Có lỗi xảy ra khi xác thực vị trí hoặc kết nối đến máy chủ. Vui lòng kiểm tra GPS và thử lại.", undefined, "error");
    } finally {
      setSubmittingAction(null);
      setSubmittingStep(null);
    }
