...
## Đợt bốn mươi lăm: tạo điều chỉnh lương

- Thêm khoản phụ cấp/thưởng/khấu trừ/điều chỉnh khác cho kỳ đang tra cứu; chọn nhân viên từ roster theo chi nhánh, nhập số tiền VND nguyên không âm và lý do.
- Dùng lại rosterService và payrollService; cần quyền đọc/quản lý kỳ và quyền API đọc danh sách nhân viên. Chỉ cho chọn người thuộc chi nhánh, không nhập mã tùy ý.
- Gửi bốn trường nghiệp vụ, backend tạo trạng thái pending. Khóa gửi/đóng khi lưu, kiểm tra phản hồi đúng kỳ/nhân viên/trạng thái; lỗi yêu cầu đóng và tải lại để tránh gửi trùng. Đóng tải lại bảng lương và điều chỉnh.
...
## Đợt bốn mươi chín: thu hồi phiếu lương

- Thu hồi từng phiếu đang được phát hành từ danh sách nhân viên trong kỳ closed/paid, dùng cùng quyền đọc kỳ/quản lý thanh toán và phạm vi phiên hiện tại.
- Xác nhận tên nhân viên, kỳ và chi nhánh trước khi gửi; có quay lại trước thao tác. Khóa phát hành/thu hồi đồng thời trong panel và khóa gửi lại sau lỗi.
- Kiểm tra phản hồi đúng kỳ/nhân viên/trạng thái withdrawn; thành công hoặc lỗi đều cho tải lại kỳ. Phiếu không còn trong danh sách phát hành của nhân viên sau thu hồi; bản đã tải/chia sẻ trước đó không bị xóa.
...
## Đợt năm mươi tư: tính và tính lại lương

- Tính/tính lại kỳ draft theo quyền đọc/quản lý, chi nhánh và version hợp lệ. Xác nhận kỳ trước khi gửi expectedVersion và Idempotency-Key UUID qua payrollService.
- Cần bản công đã khóa; backend quyết định điều kiện và công thức. Tính lại tạo revision mới, giữ kỳ nháp; không duyệt/chốt/thanh toán tự động.
- Kiểm tra revision completed đúng runId, version tăng và effectiveLines; hiển thị mã bản tính/số dòng, tải lại để xem số liệu và cảnh báo. Lỗi hoặc timeout không tự retry.
...
## Đợt năm mươi sáu: chốt kỳ lương

- Chốt kỳ review với quyền đọc/quản lý, chi nhánh và version hợp lệ. Tận dụng giao diện xác nhận duyệt để hiển thị tổng thực nhận/số dòng và xác nhận chốt riêng.
- Dùng closeRun của FE, expectedVersion và runId đã mã hóa; kiểm tra đúng kỳ/trạng thái closed/version +1, khóa gửi trùng và tải lại sau kết quả hoặc lỗi.
- Backend đối chiếu revision/checksum và số liệu đã lưu. Chốt không chuyển tiền hay tự phát hành phiếu; tải lại để dùng xuất báo cáo/phát hành theo quyền.
...
## Đợt năm mươi tám: tạo thanh toán lương nháp

- Tạo khoản nháp cho kỳ closed với quyền đọc kỳ/đọc và quản lý thanh toán, chi nhánh xác thực. Nhập phân bổ từng nhân viên, để trống người không chọn, ghi chú và xem lại tổng trước khi xác nhận.
- Kiểm tra số tiền nguyên VND dương/tổng an toàn, nhân viên thuộc effectiveLines, ghi chú tối đa 1000 ký tự. Gửi amount/lines/note/idempotencyKey UUID qua createPayment dùng chung.
- Đối chiếu phản hồi draft đúng kỳ/tổng/từng phân bổ, khóa gửi trùng, yêu cầu tải lại sau kết quả hoặc lỗi. Backend kiểm tra số dư; khoản nháp chưa ghi nhận đã trả hoặc chuyển tiền.
...
## Đợt sáu mươi: hủy và đảo thanh toán lương

- Hủy khoản nháp từ lịch sử, kể cả khi kỳ đã mở lại; tổng đã trả và trạng thái kỳ giữ nguyên. Khoản đã hủy vẫn có trong lịch sử và không xác nhận tiếp được.
- Đảo khoản confirmed trong kỳ closed/paid, xem lại phân bổ và tác động trước khi gửi. Backend tính lại tổng đã trả/trạng thái kỳ; thao tác không hoàn tiền ngân hàng.
- Dùng lại panel xác nhận và cancelPayment/reversePayment của FE; quyền đọc kỳ/đọc và quản lý thanh toán, chi nhánh xác thực. Khóa chọn/gửi trùng, kiểm tra phản hồi đúng khoản/kỳ/số tiền/phân bổ/trạng thái, tải lại cả kỳ sau kết quả hoặc lỗi, không tự retry.
- Chưa nhập ngày/chứng từ/ghi chú riêng cho chuyển trạng thái hoặc UAT thiết bị/staging. Đảo khoản cũ thiếu phân bổ chưa mở trên mobile; hủy nháp cũ không cần phân bổ.
- Kiểm tra: 313/313 test trong 57 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi mốt: ngày và chứng từ thanh toán

- Tạo nháp và xác nhận thanh toán có ngày tùy chọn YYYY-MM-DD, quy đổi đầu ngày Việt Nam UTC+7; kiểm tra ngày thật và năm nhuận, không phụ thuộc múi giờ thiết bị.
- Tạo/xác nhận/hủy/đảo hỗ trợ liên kết chứng từ HTTP/HTTPS tối đa 2000 ký tự và ghi chú tối đa 1000 ký tự. Khi chuyển trạng thái, để trống giữ metadata cũ; nội dung mới thay giá trị cũ, chưa xóa trường đã lưu. Hủy/đảo không sửa ngày thanh toán.
- Xem lại thông tin trước gửi, khóa payload và kiểm tra metadata phản hồi cùng khoản/phân bổ/trạng thái. Service FE nhận payload tùy chọn, lời gọi cũ vẫn không có body.
- Chứng từ là liên kết, chưa upload tệp hoặc mở liên kết từ app. Chưa UAT thiết bị/staging.
- Kiểm tra: 323/323 test trong 58 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi hai: tra cứu dữ liệu đối soát đầu vào

- Tra cứu theo kỳ/chi nhánh bằng getPeriodInputs của FE, không phụ thuộc kỳ đã có bảng lương. Tìm nhân viên/mã/lý do, mở chi tiết, hiển thị thêm mỗi 20 bản ghi; chỉ liệt kê bản ghi đã lưu, không suy diễn danh sách nhân sự đầy đủ.
- Hiển thị lương thỏa thuận, ngày/giờ đối soát, phụ cấp/thưởng/khấu trừ, customValues, lý do/phiên bản/người cập nhật. Phân biệt số 0 với trường chưa nhập, danh mục biến đang áp dụng và mặc định hiển thị riêng; giữ mã biến cũ.
- Kiểm tra hợp đồng phản hồi và kỳ, giữ lỗi API, bỏ kết quả khi rời màn hình/đổi phạm vi. Hiển thị editable/needsRefresh từ API, chưa chỉnh dữ liệu hoặc tự tính lại lương.
- Chưa UAT thiết bị/staging; tên nhân viên dùng dòng lương đã tải, thiếu tên thì hiện mã. Sửa hướng dẫn tạo kỳ để phản ánh luồng đồng bộ/khóa/tính hiện có trên mobile.
- Kiểm tra: 330/330 test trong 59 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi ba: chỉnh và lưu đối soát cơ bản

- Chỉnh từng bản ghi đã có: lương thỏa thuận, ngày/giờ đối soát, phụ cấp, thưởng, khấu trừ. Quyền đọc/quản lý kỳ, phân hệ HR/chi nhánh và editable từ API; lý do bắt buộc (mobile giới hạn 1000 ký tự).
- Ô trống giữ dữ liệu cũ, nhập 0 ghi đè bằng 0. Chỉ gửi trường thay đổi cùng expectedVersion/reason, giữ customValues; xem trước giá trị cũ/mới và lý do, khóa gửi trùng.
- Đối chiếu phản hồi đúng nhân viên/kỳ/version +1, giá trị đã sửa và đã giữ. Lỗi/xung đột yêu cầu tải lại, không tự retry; sau lưu tải lại kỳ và đối soát, không tự tính lương.
- Chưa thêm nhân viên chưa có bản ghi, sửa biến tùy chỉnh, hoàn tác trường về nguồn hoặc lưu hàng loạt. Chưa UAT thiết bị/staging.
- Kiểm tra: 338/338 test trong 60 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi bốn: hoàn tác đối soát về nguồn

- Trong form chỉnh đối soát, chọn “Dùng dữ liệu nguồn” cho từng trường cơ bản đã nhập, kể cả số 0; có bỏ chọn trước lưu. Ô trống vẫn giữ dữ liệu cũ.
- Xem trước trường sẽ gỡ, lý do bắt buộc, gửi clearFields cùng expectedVersion qua service hiện có. Có thể vừa sửa trường khác vừa hoàn tác, không gửi giá trị mới cho trường đã chọn gỡ.
- Kiểm tra phản hồi trường đã mất, phiên bản tăng và các giá trị khác còn nguyên. Tải lại sau kết quả/lỗi, không tự retry hoặc tính lại lương. Nguồn thực tế được xác định khi tính lại, không mặc định bằng 0.
- Chưa hoàn tác biến tùy chỉnh, thêm bản ghi hoặc lưu hàng loạt; chưa UAT thiết bị/staging.
- Kiểm tra: 343/343 test trong 61 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi lăm: đối soát biến tùy chỉnh

- Form chỉnh đối soát có các biến đang áp dụng: nhập giá trị không âm, hiển thị đơn vị/mặc định và xem trước thay đổi; phần trăm giới hạn 0–100. Ô trống giữ giá trị cũ, số 0 là ghi đè.
- Hoàn tác biến đang áp dụng đã có giá trị bằng clearFields custom.CODE. Sửa biến và hoàn tác biến được lưu riêng để tránh xung đột $set cả map với $unset nhánh con ở backend.
- Khi sửa, gửi map customValues gồm cả biến cũ không đổi để tránh mất dữ liệu. Giữ expectedVersion/lý do/quyền, kiểm tra tất cả giá trị phản hồi và tải lại sau kết quả/lỗi.
- Chưa sửa/hoàn tác biến đã ngưng áp dụng, quản trị danh mục biến, thêm bản ghi hoặc bulk; chưa UAT thiết bị/staging.
- Kiểm tra: 349/349 test trong 62 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi sáu: thêm bản ghi đối soát

- Thêm đối soát cho nhân viên chưa có bản ghi trong kỳ; lấy roster chi nhánh qua service FE, lọc phạm vi và loại bản ghi đã có/ID trùng. Tìm tên/mã, xem thêm 20 người mỗi lượt.
- Dùng lại form chỉnh đối soát cho bản ghi mới version 0; nhập ít nhất một giá trị cơ bản hoặc biến đang áp dụng và lý do. Không tự điền 0 hay mặc định vào payload.
- Xem lại, khóa gửi trùng, gửi PUT với expectedVersion 0, kiểm tra phản hồi version 1 đúng nhân viên/kỳ/giá trị. Sau kết quả/lỗi tải lại cả kỳ và đối soát; không tự retry hoặc tính lại lương.
- Chưa lưu hàng loạt/quản trị danh mục biến hoặc UAT thiết bị/staging. Cần tài khoản được API roster cho phép đọc danh sách nhân viên.
- Kiểm tra: 354/354 test trong 63 file, TypeScript và export Hermes Android/iOS qua (không phải APK/IPA).

## Đợt sáu mươi bảy: danh mục và tạo biến lương nháp

- Danh mục toàn công ty có tìm mã/tên/mô tả, lọc nháp/đang áp dụng/ngưng áp dụng, xem thêm 20 biến, đơn vị và mặc định. Kiểm tra phản hồi đúng company; trạng thái/đơn vị mới vẫn hiển thị theo API.
- Tạo nháp với quyền đọc/quản lý kỳ: mã/tên/đơn vị, mặc định và mô tả tùy chọn, xem lại phạm vi toàn công ty trước gửi. Mã không trùng danh mục đã tải, bắt đầu bằng chữ, tối đa 64 ký tự; tên 200/mô tả 1000; mặc định không âm và phần trăm tối đa 100.
- Tái sử dụng getPeriodInputVariables/createPeriodInputVariable; kiểm tra draft/version 0 và toàn bộ nội dung phản hồi, khóa gửi trùng, lỗi yêu cầu tải lại. Nháp chưa xuất hiện trong đối soát active.
