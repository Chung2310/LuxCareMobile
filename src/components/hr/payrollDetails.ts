...
export function buildPayrollDetails(
  ...
  return {
    ...
    shortageMinutes,
    shortageDays: Number(
      calculation.shortageDays ??
        attendance.shortageDays ??
        (standardDays && standardHours ? shortageMinutes / (standardHours * 60) : 0),
    ),
    ...
    netBeforeKpi: Number(calculation.netBeforeKpi ?? calculation.net ?? 0),
    systemKpiPercent: Number(calculation.systemKpiPercent ?? 100),
    appliedKpiPercent: Number(calculation.appliedKpiPercent ?? 100),
    kpiAdjustment: Number(calculation.kpiAdjustment ?? 0),
    vietnam,
    deductionBreakdown: (() => {
      ...
      const kpiAdjustment = Number(calculation.kpiAdjustment ?? 0);
      return {
        socialInsurance,
        healthInsurance,
