...
export function createNotificationService({ fetch, getAccessToken }: ServiceTransport) {
  return {
    /**
    ...
    async getNotifications(
      ...
      return await parseApiResponse<GetNotificationsResponse>(res, "Không thể tải danh sách thông báo");
    },
    ...
    async markAsRead(id: string): Promise<WebNotification> {
      ...
      const result = await parseApiResponse<{ data: WebNotification }>(res, "Không thể đánh dấu đọc thông báo");
      return result.data;
    },
    ...
    async markAllAsRead(): Promise<boolean> {
      ...
      const result = await parseApiResponse<{ status: string }>(res, "Không thể đánh dấu đọc tất cả thông báo");
    ...
    async deleteNotification(id: string): Promise<boolean> {
      ...
      const result = await parseApiResponse<{ status: string }>(res, "Không thể xóa thông báo");
      return result.status === "success";
    },
  };
}

export const notificationService = createNotificationService(browserTransport);
