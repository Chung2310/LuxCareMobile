...
Phát hành phiếu gọi POST `/api/v1/payroll/runs/:id/payslips/publish`, quyền `payroll-payment:manage`, body employeeIds tường minh. Mobile yêu cầu thêm đọc kỳ để tra cứu; chỉ chọn nhân viên trong effectiveLines của kỳ closed/paid. Backend lọc mã hợp lệ, upsert từng publication với checksum hiện hành, trả mảng docs published; Promise.all không đảm bảo rollback toàn bộ khi lỗi. App kiểm tra đúng runId/employeeId/status và đủ danh sách, chặn gửi lại trong lần xác nhận, yêu cầu tải lại sau lỗi. publishedEmployeeIds từ GET kỳ chỉ gồm publication khớp checksum có hiệu lực. Chưa có giao diện thu hồi; phát hành không gửi email hoặc push từ mobile.
